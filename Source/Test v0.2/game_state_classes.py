...
class Game_state:
    ...
    def __init__(self, time_left=60):
        ...
        self.engine = chess.engine.SimpleEngine.popen_uci('stockfish.exe')#The engine used is stockfish. It requires to have the command stockfish working on the shell
            ...
            for arrival in potential_arrivals:
                ...
                if move in self.board.legal_moves:
                    if self.can_image_correspond_to_chessboard(move,current_chessboard_image):#We only keep the move if the current image looks like this move happenned
                        ...
                        print('Failed [correspond]', uci_move)
                else:
    ...
    def register_move_if_needed(self):
        ...
        #k = cv2.waitKey(10000)                
        cv2.imwrite('Old.png', self.previous_chessboard_image)
        new_board = chessboard_detection.get_chessboard(self)
        cv2.imwrite('New.png', new_board)
    ...
    def play_next_move(self):
        ...
        if self.we_play_white:
            engine_process = self.engine.play(self.board, chess.engine.Limit(white_clock=self.time_left), ponder=True)
        else:
            engine_process = self.engine.play(self.board, chess.engine.Limit(black_clock=self.time_left), ponder=True)
