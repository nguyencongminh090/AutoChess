...
def has_square_image_changed(old_square, new_square):#If there has been a change -> the image difference will be non null -> the average intensity will be > treshold
    ...
    if diff.mean() > 7: #8 works pretty nicely but would require optimization
        return True
    else:
        return False

def is_square_empty(square, k=0): # A square is empty if its pixels have no variations
    if k == 1:
        return square.std()
    return square.std() < 38 # 10 works pretty well -> the mouse pointer is not enought to disturb (but sometimes it actually does, especially with small chessboards and big pointer)
        ...
        for column in range(8):
            ...
            if has_square_image_changed(old_square, new_square):
                ...
                if  square_was_empty == False:
                    ...
                    print('Accept Val Old:', is_square_empty(old_square, 1))
                else:
                    print('Val Old:', is_square_empty(old_square, 1))
                if  square_is_empty== False:
                    print('Accept Val New:', is_square_empty(new_square, 1))
                    potential_arrivals = np.append(potential_arrivals,square_name)
                else:
                    print('Val New:', is_square_empty(new_square, 1))
    return potential_starts, potential_arrivals

def fblack_move(img, is_white_on_bottom=False):
    potential_starts = ''
    potential_arrivals = ''
    for row in range(2):
        for column in range(8):
            square = get_square_image(row, column, img)
            square_name = convert_row_column_to_square_name(row,column,is_white_on_bottom)
            if is_square_empty(square):
                potential_starts = square_name
                break
    ...
    for row in range(3, 5):
        for column in range(8):
            square = get_square_image(row, column, img)
            square_name = convert_row_column_to_square_name(row,column,is_white_on_bottom)
            if not is_square_empty(square):
                potential_arrivals = square_name
                break
    return potential_starts + potential_arrivals
